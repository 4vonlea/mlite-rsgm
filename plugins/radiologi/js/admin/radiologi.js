...
$("#form").on("click","#no_rawat", function(event){
  ...
  var url = baseURL + '/radiologi/maxid?t=' + mlite.token;
  $.post(url, {
  } ,function(data) {
    $("#no_rawat").val(data);
  });
...
$("#form").on("click","#no_reg", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/radiologi/maxantrian?t=' + mlite.token;
...
$("#display").on("click", ".edit", function(event){
  ...
  $.post(url, {no_rawat: no_rawat} ,function(data) {
    // tampilkan data
    $("#form").html(data).show();
    var url    				= baseURL + '/radiologi/statusdaftar?t=' + mlite.token;
...
$("#form").on("click","#hapus", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/radiologi/hapus?t=' + mlite.token;
  //var no_rawat = $(this).attr("data-no_rawat");
  var no_rawat = $('input:text[name=no_rawat]').val();
  ...
  bootbox.confirm("Apakah Anda yakin ingin menghapus data ini?", function(result){
    ...
    if (result){
      ...
      $.post(url, {
        ...
        $("#display").load(baseURL + '/radiologi/display?t=' + mlite.token);
        bersih();
...
$('input:text[name=layanan]').on('input',function(e){
  var baseURL = mlite.url + '/' + mlite.admin;
  var url    = baseURL + '/radiologi/layananradiologi?t=' + mlite.token;
...
$("#rincian").on("click",".hapus_radiologi", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/radiologi/hapusradiologi?t=' + mlite.token;
  var no_rawat = $(this).attr("data-no_rawat");
  var kd_jenis_prw = $(this).attr("data-kd_jenis_prw");
