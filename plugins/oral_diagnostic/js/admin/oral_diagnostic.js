...
$("#index").on('click', '#bukaform', function(){
  var baseURL = mlite.url + '/' + mlite.admin;
  $("#form").show().load(baseURL + '/oral_diagnostic/form?t=' + mlite.token);
...
$("#form").on("click", "#simpan", function(event){
  ...
  var url = baseURL + '/oral_diagnostic/save?t=' + mlite.token;
    ...
    $.post(url,{
      ...
      if(data.status == 'success') {
        if(typeof ws != 'undefined' && typeof ws.readyState != 'undefined' && ws.readyState == 1){
          let payload = {
              'action' : 'simpan',
              'modul' : 'oral_diagnostic'
          }
          ws.send(JSON.stringify(payload));
          console.log(payload);
        } else {
          $("#display").show().load(baseURL + '/oral_diagnostic/display?t=' + mlite.token);
        }
        bersih();
        $("#status_pendaftaran").hide();
        $('#notif').html("<div class=\"alert alert-success alert-dismissible fade in\" role=\"alert\" style=\"border-radius:0px;margin-top:-15px;\">"+
        "Data pendaftaran oral diagnostic telah disimpan!"+
        "<button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-label=\"Close\">&times;</button>"+
        "</div>").show();
      }
      if(data.status == 'error') {
        $('#notif').html("<div class=\"alert alert-danger alert-dismissible fade in\" role=\"alert\" style=\"border-radius:0px;margin-top:-15px;\">"+
        "Gagal menyimpan data pendaftaran oral diagnostic!<br>"+
        data.msg+
        "<button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-label=\"Close\">&times;</button>"+
        "</div>").show();
      }
...
$("#display").on("click",".antrian", function(event){
  ...
  window.open(baseURL + '/oral_diagnostic/antrian?no_rawat=' + no_rawat + '&t=' + mlite.token);
});
...
$("#display").on("click", ".edit", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/oral_diagnostic/form?t=' + mlite.token;
  var no_rawat = $(this).attr("data-no_rawat");
  $.post(url, {no_rawat: no_rawat} ,function(data) {
    // tampilkan data
    $("#form").html(data).show();
    var url    				= baseURL + '/oral_diagnostic/statusdaftar?t=' + mlite.token;
...
$("#form").on("click","#hapus", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/oral_diagnostic/hapus?t=' + mlite.token;
  var no_rawat = $('input:text[name=no_rawat]').val();
      ...
      if (result){
        $.post(url, {
          ...
          if (res.status === 'success') {
            $("#display").load(baseURL + '/oral_diagnostic/display?t=' + mlite.token);
            bersih();
            bootbox.alert({
              message: customStyle +
...
$('#manage').on('click', '#submit_periode_rawat_jalan', function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url    = baseURL + '/oral_diagnostic/display?t=' + mlite.token;
...
$("#form_soap").on("click", "#simpan_soap", function(event){
  ...
  {else}
    ...
    var url = baseURL + '/oral_diagnostic/savesoap?t=' + mlite.token;
    ...
    spo2 : spo2
      ...
      // tampilkan data
      $("#display").hide();
      var url = baseURL + '/oral_diagnostic/soap?t=' + mlite.token;
      $.post(url, {no_rawat : no_rawat,
      }, function(data) {
        // tampilkan data
        $("#soap").html(data).show();
...
$("#soap").on("click",".hapus_soap", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/oral_diagnostic/hapussoap?t=' + mlite.token;
  var no_rawat = $(this).attr("data-no_rawat");
  var tgl_perawatan = $(this).attr("data-tgl_perawatan");
  var jam_rawat = $(this).attr("data-jam_rawat");
...
$('input:text[name=layanan]').on('input',function(e){
  var baseURL = mlite.url + '/' + mlite.admin;
  var url    = baseURL + '/oral_diagnostic/layanan?t=' + mlite.token;
...
$('input:text[name=obat]').on('input',function(e){
  var baseURL = mlite.url + '/' + mlite.admin;
  var url    = baseURL + '/oral_diagnostic/obat?t=' + mlite.token;
...
$("#form_rincian").on("click", "#simpan_rincian", function(event){
  ...
  var url = baseURL + '/oral_diagnostic/savedetail?t=' + mlite.token;
  $.post(url, {no_rawat : no_rawat,
  ...
  jml            : jml
    ...
    // tampilkan data
    $("#display").hide();
    var url = baseURL + '/oral_diagnostic/rincian?t=' + mlite.token;
    $.post(url, {no_rawat : no_rawat,
    }, function(data) {
      // tampilkan data
...
$("#rincian").on("click",".hapus_detail", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/oral_diagnostic/hapusdetail?t=' + mlite.token;
  var no_rawat = $(this).attr("data-no_rawat");
  var kd_jenis_prw = $(this).attr("data-kd_jenis_prw");
  var tgl_perawatan = $(this).attr("data-tgl_perawatan");
  var jam_rawat = $(this).attr("data-jam_rawat");
...
$("#rincian").on("click",".hapus_resep_obat", function(event){
  var baseURL = mlite.url + '/' + mlite.admin;
  event.preventDefault();
  var url = baseURL + '/oral_diagnostic/hapusresep?t=' + mlite.token;
...
$("#form").on("click","#jam_reg", function(event){
    var baseURL = mlite.url + '/' + mlite.admin;
    var url = baseURL + '/oral_diagnostic/cekwaktu?t=' + mlite.token;
    $.post(url, {
    } ,function(data) {
  ...
  ws.onmessage = function(response){
    try{
      ...
      if(output['action'] == 'simpan'){
        if(output['modul'] == 'oral_diagnostic'){
          $("#oral_diagnostic #display").show().load(baseURL + '/oral_diagnostic/display?t=' + mlite.token);
        }
      }
    }catch(e){
      console.log(e);
